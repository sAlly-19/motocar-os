    ...
    <>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 0}>
        ...
        >
          ...
          <AppText variant="h1" color={undefined} style={{ color: colors.primary, marginBottom: spacing.sm }}>Cadastro de Cliente</AppText>
          ...
          <View style={{ backgroundColor: colors['surface-container-lowest'], borderWidth: 1, borderColor: colors['outline-variant'], borderRadius: br.xl, overflow: 'hidden', marginBottom: spacing.gutter }}>
            ...
            <View style={{ padding: spacing.lg }}>
              <FormField label="Nome Completo / Razão Social" placeholder="Ex: João da Silva" value={fullName} onChangeText={(v) => { setFullName(v); clearError('fullName'); markChanged(); }} error={errors.fullName} />
            </View>
          </View>
          <View style={{ backgroundColor: colors['surface-container-lowest'], borderWidth: 1, borderColor: colors['outline-variant'], borderRadius: br.xl, overflow: 'hidden', marginBottom: spacing.gutter }}>
            <CardHeader title="Contato & Digital" icon="contact_phone" />
            <View style={{ padding: spacing.lg }}>
              <FormField label="Telefone / WhatsApp" placeholder="(00) 00000-0000" keyboardType="phone-pad" value={phone} onChangeText={(v) => { setPhone(v); clearError('phone'); markChanged(); }} error={errors.phone} />
            </View>
          </View>
          <View style={{ backgroundColor: colors['surface-container-lowest'], borderWidth: 1, borderColor: colors['outline-variant'], borderRadius: br.xl, overflow: 'hidden', marginBottom: spacing.gutter }}>
            <CardHeader title="Endereço" icon="location_on" />
            <View style={{ padding: spacing.lg }}>
              <View style={{ flexDirection: 'row', gap: spacing.sm }}><View style={{ flex: 1 }}><FormField label="CEP" placeholder="00000-000" value={zipCode} onChangeText={(v) => { setZipCode(v); markChanged(); }} /></View><View style={{ flex: 3 }}><FormField label="Logradouro" placeholder="Rua, Avenida..." value={street} onChangeText={(v) => { setStreet(v); markChanged(); }} /></View></View>
              <View style={{ flexDirection: 'row', gap: spacing.sm }}><View style={{ flex: 1 }}><FormField label="Número" placeholder="123" value={streetNumber} onChangeText={(v) => { setStreetNumber(v); markChanged(); }} /></View><View style={{ flex: 3 }}><FormField label="Bairro" placeholder="Bairro" value={neighborhood} onChangeText={(v) => { setNeighborhood(v); markChanged(); }} /></View></View>
              <View style={{ flexDirection: 'row', gap: spacing.sm }}><View style={{ flex: 2 }}><FormField label="Cidade" placeholder="Cidade" value={city} onChangeText={(v) => { setCity(v); markChanged(); }} /></View><View style={{ flex: 1 }}><FormField label="UF" placeholder="UF" value={state} onChangeText={(v) => { setState(v); markChanged(); }} /></View></View>
            </View>
          </View>
          <View style={{ gap: spacing.sm, marginTop: spacing.lg }}>
            <Button
              ...
              onPress={handleSave}
            />
            <Button variant="outline" title="Descartar Alterações" fullWidth onPress={() => router.back()} />
          </View>
        </ScrollView>
        <Dialog
